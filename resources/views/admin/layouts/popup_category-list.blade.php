        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header"><i class="fas fa-table mr-1"></i>종류 목록
                          <a style="float: right; padding-top: 0px; padding-bottom: 0px;" class="btn btn-warning" href="{{ route('admin_popup_category.create') }}">등록</a>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach ($popup_categorys as $popup_category)
                                          <tr>
                                            <td><a href="{{ route('admin_popup_category.show', $popup_category->id) }}">{{ $popup_category->id }}</a></td>
                                            <td><a id="edit" class="btn btn-warning" href="{{ route('admin_popup_category.edit', $popup_category->id) }}">수정</a></td>
                                            <td>
                                              <form action="{{ route('admin_popup_category.destroy', $popup_category->id) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
