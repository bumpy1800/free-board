        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header"><i class="fas fa-table mr-1"></i>댓글목록</div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach ($comments as $comment)
                                          <tr>
                                            <td>{{ $comment->id }}</td>
                                            <td><a href="admin_post/{{ $comment->gallery_link }}/{{ $comment->post_id }}">{{ $comment->post_title }}</a></td>
                                            <td>{{ $comment->contents }}</td>
                                            <td>{{ $comment->reg_date }}</td>
                                            <td>{{ $comment->user_id }}</td>
                                            <td>{{ $comment->nouser_name }}</td>
                                            <td><a id="edit" class="btn btn-warning" href="{{ route('admin_comment.edit', $comment->id) }}">수정</a></td>
                                            <td>
                                              <form action="{{ route('admin_comment.destroy', $comment->id) }}" method="post">
                                                @method('DELETE')
                                                @csrf
                                                <button id="del" type="submit" class="btn btn-danger">삭제</button>
                                              </form>
                                            </td>
                                          </tr>
                                        @endforeach
