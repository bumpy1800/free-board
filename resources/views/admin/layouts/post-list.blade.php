                ...
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header"><i class="fas fa-table mr-1"></i>게시물목록
                          <a style="float: right; padding-top: 0px; padding-bottom: 0px;" class="btn btn-warning" href="{{ route('admin_post.create') }}">등록테스트</a>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                          <tr>
                                            <td>{{ $post->id }}</td>
                                            <td><a href="admin_post/{{ $post->gallery_link }}/{{ $post->id }}">{{ $post->title }}</a></td>
                                            <td>{{ $post->user_id }}</td>
                                            <td>{{ $post->reg_date }}</td>
                                            <td>{{ $post->view }}</td>
                                            ...
                                            <td>{{ $post->head }}</td>
                                            <td>{{ $post->notice }}</td>
                                            <td><a id="edit" class="btn btn-warning" href="{{ route('admin_post.edit', $post->id) }}">수정</a></td>
                                            <td>
