        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header"><i class="far fa-edit"></i>로고 수정</div>
                        <div class="card-body">
                          <form name="form" action="{{ route('admin_logo.update', $Logo->id) }}" method="post" enctype="multipart/form-data">
                            @method('PATCH')
                            @csrf
                            <div class="form-group">
                              <label for="name">로고명</label>
                              <input type="text" name="name" class="form-control" id="name" value="{{ $Logo->name }}">
                              @error('name')
                                  <small style="color: red!important;" id="danger" class="form-text text-muted">{{ $message }}</small>
                              @enderror
                            </div>
                            <div class="form-group">
                              <label for="pic">로고</label>
                              <input type="file" name="pic" class="form-control" id="pic" value="{{ $Logo->pic }}">
                              @error('pic')
                                  <small style="color: red!important;" id="danger" class="form-text text-muted">{{ $message }}</small>
                              @enderror
                            </div>
