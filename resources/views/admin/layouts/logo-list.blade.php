        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    ...
                    <div class="card mb-4">
                        <div class="card-header"><i class="fas fa-table mr-1"></i>로고 목록
                          <a style="float: right; padding-top: 0px; padding-bottom: 0px;" class="btn btn-warning" href="{{ route('admin_logo.create') }}">등록테스트</a>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach ($Logo as $logo)
                                          <tr>
                                            <td>{{ $logo->id }}</td>
                                            <td><a href="{{ route('admin_logo.show', $logo->id) }}">{{ $logo->name }}</a></td>
                                            <td><a id="edit" class="btn btn-warning" href="{{ route('admin_logo.edit', $logo->id) }}">수정</a></td>
                                            <td>
                                              <form action="{{ route('admin_logo.destroy', $logo->id) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
